...
"""Tests for website/__init__.py routes and app initialization."""
...
def app_with_users(app):
    ...
    with app.app_context():
        organizer = User(firstname='Org', lastname='User', email='organizer@test.com', auth='organizer')
        presenter = User(firstname='Presenter', lastname='User', email='presenter@test.com', auth='presenter')
        attendee = User(firstname='Attendee', lastname='User', email='attendee@test.com', auth='attendee')
        restricted = User(firstname='Restricted', lastname='User', email='restricted@test.com', auth='banned')
        grader = User(firstname='Grader', lastname='User', email='grader@test.com', auth='abstract-grader')
        db.session.add_all([organizer, presenter, attendee, restricted, grader])
        db.session.commit()
        yield app
...
def test_root_route_renders_schedule(client):
    """GET / should render the schedule page as the default page."""
    response = client.get('/')
...
def test_schedule_route_shows_organizer_view(client, app_with_users):
    """GET /schedule should render for organizers."""
...
def test_restricted_user_redirected_from_dashboard(client, app_with_users):
    """Restricted users should be redirected from dashboard."""
    with client.session_transaction() as sess:
        sess['user'] = {'email': 'restricted@test.com', 'name': 'Restricted User'}
...
def test_import_csv_accepts_valid_csv(client, app_with_users):
    ...
    data = {'csv_file': (BytesIO(csv_content), 'users.csv')}
    response = client.post('/import_csv', data=data, content_type='multipart/form-data', follow_redirects=False)
    assert response.status_code == 302
    assert '/organizer-user-status' in response.location
