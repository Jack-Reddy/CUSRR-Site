...
def test_import_users_from_csv(client, organizer_user):
    ...
    response = client.post('/import_csv', data=data, content_type='multipart/form-data', follow_redirects=True)
...
def test_import_users_from_csv_unauthorized(client):
    ...
    data = {
        'csv_file': (io.BytesIO(b"firstname,lastname,email,auth\nJohn,Doe,johndoe@gmail.com,presenter"), 'users.csv')
    }
